...
import {StyleSheet,Text,View, TextInput, Button,Alert,ScrollView,TouchableOpacity,TouchableHighlight,TouchableNativeFeedback,Pressable,Switch,TouchableWithoutFeedback} from 'react-native';
...
export default function App() {
  ...
  const AlertaConfirmacion = () => {
    const confirmacion = window.confirm('¿Confrimas?');
    if (confirmacion) {
      window.alert('Confirmo');
    } else {
      window.alert('Obviamente no');
    }
  };

  const AlertaTexto = () => {
    const confirmacion = window.prompt('¿Estas aqui?', 'exactamente');
    if (confirmacion) {
      window.alert(`Obvio, ${confirmacion}`);
    } else {
      window.alert('Respondee!!!');
    }
  };
  ...
  const AlertaTimpo = () => {
    setTimeout(() => {
      window.alert('Pasaron 5 segundos');
    }, 5000);
  };
 ...
 <Button title='Alerta basica' onPress={AlertaBasica}></Button>
      ...
      <Button title='Alerta Promp' onPress={AlertaTexto}></Button>
